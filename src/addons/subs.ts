...
function subsText(title: string, year: string, tag: string, subs: WyzieSub[]): string {
  if (!Array.isArray(subs) || subs.length === 0) {
    return `No subtitles found for *${title}*${year ? ` (${year})` : ""}${tag}.`;
  }
...
registerAddon({
  ...
  register(bot: Bot, env: Env) {
    async function fetchSubs(tmdbId: number, language: string, season?: number, episode?: number) {
      const params = new URLSearchParams({ id: String(tmdbId), key: env.WYZIE_API_KEY!, language });
      if (season !== undefined && episode !== undefined) {
        params.set("season", String(season));
        params.set("episode", String(episode));
      }
      return fetch(`https://sub.wyzie.io/search?${params}`).then(r => r.json() as Promise<WyzieSub[]>);
    }

    async function fetchTv(tmdbId: number) {
      return fetch(`https://api.themoviedb.org/3/tv/${tmdbId}?api_key=${env.TMDB_API_KEY}`)
    ...
    bot.callbackQuery(/^subs:se:(\d+):(\d+):(\w{2})$/, async (ctx) => {
      ...
      const seasonData = await fetch(`https://api.themoviedb.org/3/tv/${tmdbId}/season/${season}?api_key=${env.TMDB_API_KEY}`)
        .then(r => r.json() as Promise<TmdbSeasonDetails>);
